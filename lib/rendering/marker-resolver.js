...
class MarkerResolver {
  ...
  _handle(infos, kind) {
    const severity = infos.severity
    if (severity === MIN_SEVERITY && !this._includeAllSeverities) return ''
    const result = this._isterminal
      ? this._determineTerminalMarkerSymbol(infos, severity, kind)
      : this._determineBrowserMarkerSymbol(infos, severity, kind)
  ...
  _determineBrowserMarkerSymbol(infos, severity, kind) {
    const symbol = (
        kind === 'deopt' ? DEOPTSYMBOL
      : kind === 'ic' ? ICSYMBOL
      : CODESYMBOL
